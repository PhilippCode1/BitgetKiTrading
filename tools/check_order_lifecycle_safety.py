...
def analyze() -> dict[str, object]:
    ...
    doc = (
        ROOT / "docs" / "production_10_10" / "order_lifecycle_exit_emergency_safety.md"
    )
    ...
    if live_doc.is_file():
        ...
        if "order" not in text or "idempot" not in text:
            issues.append(
                {
                    "severity": "error",
                    "code": "live_broker_order_lifecycle_missing",
                    "message": "Live-Broker-Doku referenziert Order-Lifecycle/Idempotency nicht.",
                    "path": str(live_doc),
                }
            )
    else:
    ...
    if main_console.is_file():
        ...
        if (
            "order-state" not in text
            and "order state" not in text
            and "order-lifecycle" not in text
        ):
            issues.append(
                {
                    "severity": "error",
                    "code": "main_console_order_state_missing",
                    "message": "Main-Console-Doku referenziert Order-States nicht.",
    ...
    if no_go.is_file():
        ...
        if "unknown" not in text or "duplicate" not in text:
            issues.append(
                {
                    "severity": "error",
                    "code": "no_go_unknown_duplicate_missing",
                    "message": "No-Go-Regeln erwaehnen unknown submit state / duplicate orders nicht ausreichend.",
                    "path": str(no_go),
                }
            )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prueft Order-Lifecycle-Safety-Artefakte."
    )
    ...
    if args.json:
        ...
        print(
            f"check_order_lifecycle_safety: ok={str(payload['ok']).lower()} errors={payload['error_count']} warnings={payload['warning_count']}"
        )
