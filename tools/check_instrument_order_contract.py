...
CONTRACT_PATH = (
    ROOT / "shared" / "python" / "src" / "shared_py" / "bitget" / "order_contract.py"
)
NO_GO_PATH = ROOT / "docs" / "production_10_10" / "no_go_rules.md"
ASSET_UNIVERSE_DOC_PATH = (
    ROOT / "docs" / "production_10_10" / "bitget_asset_universe.md"
)
...
def analyze() -> dict[str, Any]:
    ...
    if not DOC_PATH.is_file():
        _issue(
            ...
            severity="error",
            code="doc_missing",
            message="Instrument-Contract-Doku fehlt.",
            path=DOC_PATH,
        )
    if not CONTRACT_PATH.is_file():
        _issue(
            ...
            message="order_contract.py fehlt.",
            path=CONTRACT_PATH,
        )
    ...
    if ASSET_UNIVERSE_DOC_PATH.is_file():
        ...
        _issue(
            ...
            message="Asset-Universe-Doku fehlt.",
            path=ASSET_UNIVERSE_DOC_PATH,
        )
    if MAIN_CONSOLE_DOC_PATH.is_file():
        ...
        has_block_reason = (
            ("blockgrund" in text) or ("blockgruend" in text) or ("blockgr" in text)
        )
