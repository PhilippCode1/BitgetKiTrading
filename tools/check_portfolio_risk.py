...
def analyze() -> dict[str, object]:
    ...
    module = (
        ROOT / "shared" / "python" / "src" / "shared_py" / "portfolio_risk_controls.py"
    )
    ...
    if doc.is_file():
        ...
        if "multi_asset_order_sizing_margin_safety.md" not in text:
            issues.append(
                {
                    "severity": "error",
                    "code": "order_sizing_doc_reference_missing",
                    "message": "Order-Sizing-Doku wird nicht referenziert.",
                    "path": str(doc),
                }
            )
    ...
    if main_console.is_file():
        ...
        if "portfolio-risiko" not in text and "portfolio risiko" not in text:
            issues.append(
                {
                    "severity": "error",
                    "code": "main_console_portfolio_missing",
                    "message": "Main-Console-Doku erwaehnt Portfolio-Risk nicht.",
    ...
    if no_go.is_file():
        ...
        if "portfolio" not in text:
            issues.append(
                {
                    "severity": "error",
                    "code": "no_go_portfolio_missing",
                    "message": "No-Go-Regeln erwaehnen Portfolio-Risk nicht.",
                    "path": str(no_go),
                }
            )
    else:
