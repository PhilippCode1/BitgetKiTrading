...
def analyze() -> dict[str, object]:
            ...
            issues.append(
                {
                    "severity": "error",
                    "code": "nav_missing",
                    "message": "Main-Console-Navigation enthält safety-center nicht.",
                    "path": str(nav),
                }
            )
    else:
    ...
    if main_console_doc.is_file():
        if (
            "sicherheitszentrale"
            not in main_console_doc.read_text(encoding="utf-8").lower()
        ):
            issues.append(
                {
                    "severity": "error",
                    "code": "main_console_doc_missing_center",
                    "message": "Main-Console-Doku erwähnt Sicherheitszentrale nicht.",
    ...
    if no_go.is_file():
        ...
        if "unknown" not in no_go_text or "kill-switch" not in no_go_text:
            issues.append(
                {
                    "severity": "error",
                    "code": "no_go_coverage_missing",
                    "message": "No-Go-Regeln decken kritische Safety-Center-Fälle nicht ausreichend ab.",
                    "path": str(no_go),
                }
            )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prüft Main-Console-Sicherheitszentrale-Artefakte."
    )
