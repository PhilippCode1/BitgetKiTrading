...
SCHEMAS_EXEC_AUTH_NONE: tuple[tuple[str, Path], ...] = (
    (
        ...
        ROOT / "shared" / "contracts" / "schemas" / "assistant_turn.schema.json",
    ),
    (
        "operator_explain.schema.json",
        ROOT / "shared" / "contracts" / "schemas" / "operator_explain.schema.json",
    ),
)


def _schema_requires_execution_authority_none(
    label: str, path: Path
) -> list[dict[str, str]]:
...
def analyze() -> dict[str, object]:
    ...
    schema_doc = (
        ROOT / "docs" / "production_10_10" / "ai_operator_assistant_prompt_schema.md"
    )
    ...
    py_contract = (
        ROOT / "shared" / "python" / "src" / "shared_py" / "ai_operator_assistant.py"
    )
    ...
    if op_err.is_file():
        ...
        issues.append(
            {
                "severity": "error",
                "code": "operator_explain_guard_missing",
                "message": "operator-explain-errors.ts fehlt.",
                "path": str(op_err),
            }
        )
        ...
        issues.append(
            {
                ...
                "message": "safety-diagnosis-errors.ts fehlt.",
                "path": str(saf_err),
            }
        )
    ...
    for panel, code in (
        (op_panel, "operator_panel_degraded_missing"),
        (saf_panel, "safety_panel_degraded_missing"),
    ):
        if panel.is_file():
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prüft Safety-Vertrag des KI-Operator-Assistenten."
    )
