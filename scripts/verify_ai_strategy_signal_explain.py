#!/usr/bin/env python3
"""
Verifikation der KI-Strecke „Strategie-Signal-Erklaerung“ (zweiter E2E-Pfad).

Modi wie verify_ai_operator_explain.py:
  orchestrator: POST /llm/analyst/strategy_signal_explain
  gateway: POST /v1/llm/operator/strategy-signal-explain

Exit 0 wenn HTTP 200 und result.strategy_explanation_de nicht leer.
"""
...
def main() -> int:
    ...
    body: dict[str, object] = {
        "signal_context_json": {
            "signal_id": "verify_ai_strategy_signal_explain",
            "symbol": "BTCUSDT",
            "direction": "long",
            "verify_script": "verify_ai_strategy_signal_explain.py",
        },
        "focus_question_de": "Fasse die wichtigsten Felder knapp zusammen.",
    }

    print("=== verify_ai_strategy_signal_explain ===")
    ...
    if args.mode == "orchestrator":
        ...
        url = f"{base}/llm/analyst/strategy_signal_explain"
        print(f"POST {url}")
        print(f"X-Internal-Service-Key={'yes' if ikey else 'no'}")
        try:
    ...
    expl = result.get("strategy_explanation_de")
    if not isinstance(expl, str) or not expl.strip():
        print("FAIL empty strategy_explanation_de", file=sys.stderr)
        return 1
