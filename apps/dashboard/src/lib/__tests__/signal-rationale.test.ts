...
describe("signal-rationale", () => {
  it("summarizeNoTradeReasons lists abstention and universal blocks", () => {
    ...
    expect(lines.some((l) => l.includes("Abstention"))).toBe(true);
    expect(lines.some((l) => l.includes("Governor (universal hard)"))).toBe(
      true,
    );
    expect(lines.some((l) => l.includes("Budget-Cap"))).toBe(true);
    expect(lines.some((l) => l.includes("nicht verifiziert"))).toBe(true);
  });
  ...
  it("summarizeTradeRationale returns playbook and exit when allow_trade", () => {
    ...
    expect(lines.some((l) => l.includes("Playbook"))).toBe(true);
    expect(lines.some((l) => l.includes("scale_out"))).toBe(true);
    expect(lines.some((l) => l.includes("live-mirror-eligible"))).toBe(true);
    expect(lines.some((l) => l.includes("Telegram"))).toBe(true);
  });

  it("summarizeTradeRationale empty when not allow_trade", () => {
    expect(summarizeTradeRationale({ trade_action: "do_not_trade" })).toEqual(
      [],
    );
  });
  ...
  it("summarizeNoTradeReasons covers stop floor and live broker block branches", () => {
    ...
    expect(lines.some((l) => l.includes("minimal ausführbaren Abstand"))).toBe(
      true,
    );
    expect(lines.some((l) => l.includes("Spot-Instrument"))).toBe(true);
    expect(lines.some((l) => l.includes("live-mirror-eligible"))).toBe(true);
    expect(lines.some((l) => l.includes("Live-Broker blockierte"))).toBe(true);
  });
  ...
  it("summarizeNoTradeReasons covers ood, uncertainty and fragility branches", () => {
    ...
    expect(lines.some((l) => l.includes("OOD-Alert"))).toBe(true);
    expect(lines.some((l) => l.includes("Hohe Modell-Unsicherheit"))).toBe(
      true,
    );
    expect(lines.some((l) => l.includes("Stop-Fragilität hoch"))).toBe(true);
  });
  ...
  it("summarizeTradeRationale includes operator gate and governor clear branch", () => {
    ...
    expect(lines.some((l) => l.includes("Operator-Gate"))).toBe(true);
    expect(
      lines.some((l) => l.includes("Stop-Fragilität/Ausführbarkeit")),
    ).toBe(true);
    expect(
      lines.some((l) => l.includes("Governor: keine Live-Execution-Blocker")),
    ).toBe(true);
    expect(lines.some((l) => l.includes("Operator-Release"))).toBe(true);
  });
  ...
  it("summarizeTradeRationale includes live block reasons when present", () => {
    ...
    expect(
      lines.some((l) => l.includes("Live-Execution durch Governor blockiert")),
    ).toBe(true);
    expect(
