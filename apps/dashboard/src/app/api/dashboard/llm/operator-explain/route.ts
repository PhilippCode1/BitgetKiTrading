...
export async function POST(req: Request) {
  ...
  const ctxRaw = (body as { readonly_context_json?: unknown })
  ...
  if (ctxRaw !== undefined && ctxRaw !== null) {
    if (typeof ctxRaw !== "object" || Array.isArray(ctxRaw)) {
      return NextResponse.json(
        {
          detail: "readonly_context_json must be a JSON object when provided.",
        },
        { status: 400 },
      );
    }
    readonly_context_json = ctxRaw as Record<string, unknown>;
  }

  const ctxBytes = readonlyContextJsonUtf8ByteLength(readonly_context_json);
  if (ctxBytes > OPERATOR_EXPLAIN_CONTEXT_JSON_MAX_BYTES) {
    return NextResponse.json(
      {
        detail: {
          code: "CONTEXT_JSON_TOO_LARGE",
          message: `readonly_context_json exceeds ${OPERATOR_EXPLAIN_CONTEXT_JSON_MAX_BYTES} UTF-8 bytes (${ctxBytes}).`,
        },
      },
      { status: 400 },
    );
  ...
  try {
    res = await fetchGatewayUpstream(
      ...
      {
        method: "POST",
        body: JSON.stringify({ question_de: qt, readonly_context_json }),
