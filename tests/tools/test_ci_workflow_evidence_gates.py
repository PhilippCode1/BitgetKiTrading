...
def test_ci_python_job_includes_matrix_and_preflight_gates() -> None:
    ...
    assert (
        "check_10_10_evidence.py" in text
    ), "CI muss tools/check_10_10_evidence.py ausfuehren"
    assert (
        "check_live_broker_preflight.py" in text
    ), "CI muss Live-Broker-Preflight strikt ausfuehren"
    assert "Evidence-Matrix 10/10" in text or "check_10_10_evidence" in text
    assert (
        "--check-report" in text
    ), "CI muss evidence_status_report.md gegen die Matrix pruefen"
...
def test_ci_evidence_matrix_step_before_live_broker_preflight() -> None:
    ...
    assert (
        c10 < lbp
    ), "check_10_10_evidence.py muss vor check_live_broker_preflight.py vorkommen"
...
def test_ci_python_job_ruff_step_includes_ai_operator_assistant_safety() -> None:
    ...
    assert (
        "check_ai_operator_assistant_safety.py" in chunk
...
def test_ci_python_job_runs_production_env_template_security_after_install_packages() -> (
    None
):
    ...
    assert (
        sec != -1
    ), "check_production_env_template_security.py muss in ci.yml vorkommen"
    assert install != -1, "Install-Step muss in ci.yml vorkommen"
    assert (
        sec > install
    ), "ENV-Template-Security-Gate soll nach Install packages (Python-Job) stehen"
...
def test_ci_compose_healthcheck_uses_local_publish_overlay() -> None:
    ...
    assert (
        chunk.count("docker-compose.local-publish.yml") >= 2
...
def test_ci_release_approval_gate_runs_check_release_approval_gates() -> None:
    ...
    chunk = text[start:]
    assert (
        "check_release_approval_gates.py" in chunk
