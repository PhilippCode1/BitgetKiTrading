...
from tools.check_bitget_key_permission_evidence import (
    build_template,
    ...
    payload.update(
        {
            ...
            "owner_review": {
                "signoff": True,
                "reviewer": "Philipp",
                "reviewed_at": "2026-04-26T00:10:00Z",
            },
            "status": "verified",
...
def test_unredacted_secret_like_fields_block() -> None:
    assert secret_surface_issues({"api_key": "real-looking-value"}) == [
        "secret_like_field_not_redacted:api_key"
    ]
    assert (
        secret_surface_issues(
            {"api_key": "[REDACTED]", "api_secret": "not_stored_in_repo"}
        )
        == []
    )
