...
def test_shadow_profile_is_real_host_and_non_live_execution() -> None:
    ...
    assert data["GATEWAY_JWT_SECRET"] == "shadow_ex_only_gateway_jwt_32_chars_____"
    assert data["INTERNAL_API_KEY"] == "shadow_ex_only_internal_key_32_chars_______"
    assert data["NEXT_PUBLIC_ADMIN_USE_SERVER_PROXY"] == "true"
    assert data["STRATEGY_EXEC_MODE"] == "manual"
...
def test_production_profile_is_real_host_and_fake_free() -> None:
    ...
    assert data["GATEWAY_ENFORCE_SENSITIVE_AUTH"] == "true"
    assert data["GATEWAY_ALLOW_LEGACY_ADMIN_TOKEN"] == "false"
    assert data["GATEWAY_JWT_SECRET"] == "prod_ex_only_gateway_jwt_32_chars_______"
    assert data["INTERNAL_API_KEY"] == "prod_ex_only_internal_key_32_chars________"
