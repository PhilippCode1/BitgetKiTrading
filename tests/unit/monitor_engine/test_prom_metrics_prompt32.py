...
def test_prompt32_metric_names() -> None:
    ...
    assert (
        SHADOW_LIVE_ASSESSMENT_MISMATCH_24H._name
        == "shadow_live_assessment_mismatch_24h"
    )
    ...
    assert (
        SIGNAL_SPECIALIST_DISAGREEMENT_RATIO_24H._name
        == "signal_specialist_disagreement_ratio_24h"
    )
    ...
    assert (
        MONITOR_ENGINE_TICK_DURATION_SECONDS._name
        == "monitor_engine_tick_duration_seconds"
    )
    assert (
        SIGNAL_PIPELINE_LAG_P95_SECONDS_1H._name == "signal_pipeline_lag_p95_seconds_1h"
    )
    assert (
        GATEWAY_MANUAL_ACTION_AUTH_FAILURES_1H._name
        == "gateway_manual_action_auth_failures_1h"
    )
    assert (
        EXECUTION_DRIFT_OR_SHADOW_DECISIONS_24H._name
        == "execution_drift_or_shadow_decisions_24h"
    )
    assert COMMERCE_LEDGER_LINES_1H._name == "commerce_ledger_lines_1h"
    assert (
        COMMERCE_BILLING_AUTH_FAILURES_1H._name == "commerce_billing_auth_failures_1h"
    )
