...
def test_shadow_mismatch_blocks_submit_via_strategy_gate() -> None:
    decision = evaluate_live_preflight(replace(_ctx(), strategy_evidence_ok=False))
    assert decision.submit_allowed is False
    assert "strategy_evidence_missing_or_invalid" in decision.blocking_reasons
