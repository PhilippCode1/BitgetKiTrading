...
def test_orderbook_missing_blocks_submit() -> None:
    decision = evaluate_live_preflight(replace(_ctx(), orderbook_present=False))
    assert decision.submit_allowed is False
    assert "orderbook_missing" in decision.blocking_reasons
