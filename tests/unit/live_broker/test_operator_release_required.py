...
def test_operator_release_missing_blocks_submit() -> None:
    decision = evaluate_live_preflight(replace(_ctx(), owner_approved=False))
    assert decision.submit_allowed is False
    assert "owner_approval_missing" in decision.blocking_reasons
