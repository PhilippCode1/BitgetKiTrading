...
def test_market_data_stale_blocks_submit() -> None:
    decision = evaluate_live_preflight(replace(_ctx(), data_quality_status="stale"))
    assert decision.submit_allowed is False
    assert "data_quality_not_pass" in decision.blocking_reasons
