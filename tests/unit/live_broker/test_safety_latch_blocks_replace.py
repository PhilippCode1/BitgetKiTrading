...
def test_safety_latch_blocks_replace_like_mutation_path() -> None:
    decision = evaluate_live_preflight(replace(_ctx(), safety_latch_active=True))
    assert decision.submit_allowed is False
    assert "safety_latch_active" in decision.blocking_reasons
