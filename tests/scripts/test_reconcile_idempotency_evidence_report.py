...
def test_external_template_must_fail_closed_and_cover_required_blockers() -> None:
    ...
    assert set(REQUIRED_EXTERNAL_BLOCKERS).issubset(
...
def test_order_and_reconcile_scenarios_are_live_blocking() -> None:
    ...
    assert (
        payload["order_idempotency_assertions"]["scenario_count"]
        >= payload["order_idempotency_assertions"]["expected_count"]
    )
    assert (
        payload["reconcile_assertions"]["scenario_count"]
        >= payload["reconcile_assertions"]["expected_count"]
    )
