...
def test_recovery_blocks_live_until_reconcile_clean() -> None:
    decision = evaluate_live_preflight(
        replace(
            _ctx(),
            recovery_reconcile_required=True,
        )
    )
    assert decision.submit_allowed is False
    assert "recovery_reconcile_required" in decision.blocking_reasons
