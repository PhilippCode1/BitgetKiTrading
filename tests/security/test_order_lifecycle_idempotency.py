...
def test_timeout_yields_unknown_submit_state() -> None:
    state, _ = evaluate_submit_safety(
        OrderSubmitContext(
            "exec-1", "idem-1", "cid-1", set(), "submit_prepared", "timeout"
        )
    )
...
def test_unknown_submit_state_blocks_new_opening() -> None:
    state, reasons = evaluate_submit_safety(
        OrderSubmitContext(
            "exec-1", "idem-1", "cid-1", set(), "unknown_submit_state", "ack"
        )
    )
    assert state == "blocked"
...
def test_retry_without_reconcile_blocks() -> None:
    state, reasons = evaluate_submit_safety(
        OrderSubmitContext(
            "exec-1", "idem-1", "cid-1", set(), "reconcile_required", "ack"
        )
    )
    assert state == "blocked"
...
def test_duplicate_client_order_id_blocks() -> None:
    state, reasons = evaluate_submit_safety(
        OrderSubmitContext(
            "exec-1", "idem-1", "cid-dup", {"cid-dup"}, "submit_prepared", "ack"
        )
    )
    assert state == "blocked"
...
def test_exchange_reject_sets_rejected_failed_path() -> None:
    state, reasons = evaluate_submit_safety(
        OrderSubmitContext(
            "exec-1", "idem-1", "cid-1", set(), "submit_prepared", "reject"
        )
    )
