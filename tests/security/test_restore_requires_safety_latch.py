...
def test_restore_requires_safety_latch_or_recovery_latch() -> None:
    decision = evaluate_live_preflight(replace(_ctx(), recovery_latch_active=True))
    assert decision.submit_allowed is False
    assert "recovery_latch_active" in decision.blocking_reasons
