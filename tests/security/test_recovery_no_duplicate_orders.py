...
def test_unknown_event_state_blocks_live_to_prevent_duplicates() -> None:
    decision = evaluate_live_preflight(replace(_ctx(), event_state_known=False))
    assert decision.submit_allowed is False
    assert "event_state_unknown" in decision.blocking_reasons
