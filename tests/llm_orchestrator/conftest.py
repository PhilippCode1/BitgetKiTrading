...
def _llm_orch_required_secrets_matrix(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    Pflicht-ENV fuer local laut config/required_secrets_matrix.json (* = alle Services).
    Ohne diese Werte scheitert bootstrap_from_settings beim Import von create_app().
    Einzeltests ueberschreiben per monkeypatch.setenv wo noetig.
    """
