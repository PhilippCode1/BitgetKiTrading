...
- Im GitHub-Repo unter **Settings → Branches → Branch protection** für `main` (und ggf. `master`) mindestens aktivieren:
  ...
  - **Require status checks** vor dem Merge: exakt die **Anzeigenamen** aus `.github/workflows/ci.yml` (Workflow-Name: `ci`, Feld `jobs.<id>.name`), in GitHub typisch als `ci / <name>`:
    - `ci / Python (Ruff, pytest, contracts, modul_mate, pip-audit, …)` — Job-Id `python`
    - `ci / Dashboard (types, Jest, build, pnpm audit)` — Job-Id `dashboard`
    - `ci / Docker Compose + Health + KI-Smoke + Playwright E2E` — Job-Id `compose_healthcheck`
    - `ci / Unified Release Gate (all jobs + FREEZE + version lock)` — Job-Id `release-approval-gate` (zieht `tools/check_release_approval_gates.py` + Freeze-Matrix; **für abgesichertes Merging zwingend**).
  - **Contract:** `pytest tests/unit/docs/test_ci_release_gates_doc_contract.py -q` gleicht diese Zeilen mit `ci.yml` ab (keine Drift zwischen Doku und Workflow).
...
- **Lokal im Repo (blockierend in CI):** `scripts/branch_protection_ci_evidence_report.py --strict` prüft, dass **alle** `MANDATORY`-Job-Ids aus `tools/check_github_branch_protection` in **`.github/workflows/ci.yml`** vorkommen und legt `reports/branch_protection_ci_evidence.{md,json}` an. Ersetzt **keinen** API-Nachweis der Remote-Branch-Policy; das externe Owner-Template bleibt in der Evidence-Matrix `implemented` / `external_required` bis `verified`.
...
| Alert-Routing (Struktur)                  | `python tools/verify_alert_routing.py --strict` mit `infra/observability/alertmanager.yml.example` und `prometheus-alerts.yml`; in CI mit `--report-md reports/alert_routing_verify.md`. Echter Kanal-/Drill-Nachweis bleibt extern (siehe `05_alert_routing_and_incident_drill.md`).                                                                                                                                                                                                                                                                                                                                               |
...
| API-Gateway Unit (Lifespan)               | `tests/unit/api_gateway/conftest.py` setzt `BITGET_SKIP_MIGRATION_LATCH=1`, damit `TestClient` ohne laufendes Postgres die Migrationssonde im App-Lifespan umgeht (Routen-/Auth-Unit); **kein** Ersatz fuer Integrations- oder E2E-Tests mit echter DB.                                                                                                                                                                                                                         |
