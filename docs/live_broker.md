...
- **Execution-Guards (Preflight + Post-Preflight):** family-aware Katalog-Checks (`shared_py.bitget.metadata.preflight_order`), danach deterministische Guards fuer Market-Spread, Preset-Stop-Abstand, Reduce-only vs. Exchange-Position, Replace-Groesse. In Shadow/Staging/Production muessen `LIVE_BROKER_BLOCK_LIVE_WITHOUT_EXCHANGE_TRUTH=true`, `LIVE_BLOCK_SUBMIT_ON_RECONCILE_FAIL=true`, `LIVE_ENABLE_PRE_FLIGHT_LIQUIDITY_GUARD=true` und `LIVE_LIQUIDITY_GUARD_STRICT_NO_CACHE=true` explizit gesetzt sein; `tools/validate_env_profile.py --template` blockiert sonst bereits die Template-/Profilfreigabe. Konfiguration: `LIVE_PREFLIGHT_*`, `LIVE_EXECUTION_MAX_SPREAD_HALF_BPS_MARKET`, `LIVE_PRESET_STOP_*`, `LIVE_REQUIRE_EXCHANGE_POSITION_FOR_REDUCE_ONLY`, `LIVE_BLOCK_SUBMIT_ON_RECONCILE_*`. Alerts/Audit: `docs/emergency_runbook.md`.
- **Fail-Closed-Evidence:** `tools/check_live_broker_preflight.py --evidence-json docs/production_10_10/live_broker_fail_closed_evidence.template.json --strict` prueft externen Staging-/Shadow-Drill fuer Provider-/Redis-/DB-/Timeout-/Exchange-Truth-Failures, Audit, Alert und Main-Console-Gate-State. Das Template bleibt absichtlich `FAIL`, bis echte Evidence vorhanden ist.
